...
学生问答学习信号进入学生档案辅助分析前，还必须满足服务端计算准入且 `schemaVersion=qa-learning-signal-v1`。教师测试、匿名或身份未确认的课堂问答、不可用、不安全、结构异常、日期无效和旧版记录不得进入公开证据，只能以最小原因摘要进入教师侧 `blockedEvidence`。单条来源问答不能提高掌握度、分数或形成强结论；同知识点重复信号可形成 `supported` 但仍是辅助证据，已确认的非问答证据优先，冲突只生成教师复核备注。
...
- 学生/家长界面只显示“AI生成”或可用状态；问答响应不得包含学习信号、供应商、模型、路由、原始输出、prompt、debug、准入或阻断元数据。
