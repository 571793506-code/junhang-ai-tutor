...
- 学生 AI 问答由一次 Terra 低推理文本调用生成 `studentAnswer + learningSignal`，安全且结构有效时即时返回，不经教师逐条预审，也不调用 Sol。
- 问答 actor、身份和准入由服务端计算；只有 `qa-learning-signal-v1` 合格记录进入档案辅助分析，普通端不接收学习信号或内部模型路由元数据。
