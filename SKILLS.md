...
- 模型原始输出必须由服务层完成 parse、normalize、validate、repair、安全检查和按角色过滤字段；学生 AI 问答通过这些检查后可即时返回，不需要教师逐条预审。
- 只有学生身份已确认、安全通过、结构有效且成功返回的问答学习信号可以进入学生档案辅助分析；教师测试、匿名或身份未确认的课堂问答，以及失败、敏感或结构异常记录不得进入。
- 单次问答不能形成掌握、退步或薄弱点等强结论；生成、批改、周/月档案、阶段报告、家长摘要和正式打印/PDF 仍按教师复核或确认流程处理。
...
- 学生、家长、课堂平板和公共屏不展示模型名、供应商、API 地址、内部 prompt、密钥、调试字段或需要教师复核但尚未确认的内容。
