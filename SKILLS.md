...
- 学生 AI 问答最多执行一次 Terra 低推理文本调用；Terra 不可用时调用数为 0，且绝不调用 Sol。实际调用成功时，同一次调用生成 `studentAnswer + learningSignal`，安全且结构有效时即时返回，不经教师逐条预审。
- 模型输出结构化 `safetyStatus`；服务端校验和规范化信号，并计算最终 actor、身份、可用性、结构有效性和准入。只有 `qa-learning-signal-v1` 合格记录进入档案辅助分析，普通端不接收学习信号或内部模型路由元数据。
- 单条来源问答不能提高掌握度、分数或形成强结论；重复信号仍是辅助证据。生成、批改、周/月档案、阶段报告、家长摘要和正式打印/PDF 继续按教师复核或确认流程处理。
...
| 方案拷问类 | `skills/project-grill-review/SKILLS.md` | Grill me、方案压力测试、计划审查、架构取舍和实施前追问 |

## 全局规则

- 核心逻辑必须沉淀到 API、服务层、脚本、共享工具或数据契约，不能只写在 Web 页面状态里。
- 外部 skills 只作为参考来源；进入项目时必须改写为本仓库模块规则，不直接安装、不照搬、不绕过任务级复核和多端可见性边界。
- 方案拷问类任务先查项目文档、代码、测试和官方资料，不能把代码库可回答的问题抛给用户；需要用户决定时一次只问一个问题，并给出推荐答案。
- 学生、家长、课堂平板和公共屏不展示模型名、供应商、API 地址、内部 prompt、密钥、调试字段、问答学习信号、准入/阻断状态或需要教师复核但尚未确认的内容。
- 教师端可以展示复核状态、服务状态和运维摘要，但不能暴露密钥、数据库连接或完整内部配置。
- 资料、生成、批改、档案和问答进入持久化或对外展示前必须检查乱码、字段完整性、权限可见性，并按任务类型处理复核状态；问答学习信号还必须执行身份和准入过滤。
