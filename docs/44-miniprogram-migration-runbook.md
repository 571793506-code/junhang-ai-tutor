...
- 小程序 AI 问答复用与 Web/课堂平板相同的服务端 API 契约，不在端侧拼 prompt、计算 actor/身份/准入或聚合档案证据。
- 学习者问答响应不接收 `learningSignal`、供应商、模型、路由、raw、prompt、debug、准入或阻断元数据。
...
- AI问答只展示“AI生成”、可用状态和服务端白名单回答，不接收或计算学习信号，不展示模型、供应商或内部路由。
- 档案页只展示教师复核并发布后的内容，不展示内部问答证据包或准入/阻断状态。
...
- 平板端 AI问答由学生提问，服务端确认课堂学生身份后按同一 API 契约直接回答或引导思路；端侧不判断问答是否进入档案。
