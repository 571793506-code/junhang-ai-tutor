...
本文用于当前仓库进入暂停小程序、准备飞书机器人迁移阶段后的工作区清理、任务收口和提交判断。它参考“飞书 AI 课后辅导机器人”项目中的工作区守护者做法，但不复制旧项目实现。
...
4. 按下面四类分组：
   - `收口提交`：规则、迁移边界、踩坑清单、已验证脚本或稳定服务层能力。
...
- `docs/50-feishu-bot-migration-prep.md`
- `docs/51-project-pitfall-review.md`
- `docs/52-workspace-guardian.md`
- 只涉及迁移边界、踩坑复盘、命令风险、Git 可追踪性的 `.gitignore` 更新
...
这些内容可以作为历史快照保留，但后续迁移到飞书机器人时只参考经验，不直接复制不稳定实现。
