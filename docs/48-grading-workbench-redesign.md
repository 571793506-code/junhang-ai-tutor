...
4. 可选深度审查：显式开启时由 MiniMax M3 检查 OCR 证据、逐题判断、总分和风险，再由 GPT5.5 拦截低置信、漏页、异常扣分、模型分歧和归档风险。
5. 教师确认：逐题确认后才归档。
...
4. 显式开启深度批改审查时，执行 MiniMax M3 二次审计和 GPT5.5 高级审查。
5. 教师逐题确认后归档。
...
- `grading`：参考答案准备和主批改中。
- `auditing`：显式开启深度批改审查后，MiniMax 二审和 GPT5.5 高级审查中。
- `reviewing`：等待教师逐题复核。
- `archived`：已归档。
- `failed`：识别或批改失败。
